...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let movie = Movie(id: id, name: name, thubnailURL: thumbnail, categories: categories, genre: .AllGenres, year: year, rating: "PG-13", numberOfSeasons: numSeasons, currentEpisode: defEpiInfo, defaultEpisodeInfo: defEpiInfo, creators: creators, cast: cast, moreLikeThisMovies: moreLikeThis, trailers: allTrailers, previewImageName: "", episode: allEpisodes, promotionHeadLine: headline)
...
func generateTrailers(_ count: Int) -> [Trailer] {
    ...
    for _ in 0..<count {
        ...
        let trailer = Trailer(id: id, name: name, videoURL: exampleVideoURL, thubnailImageURL: thumbnail)
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episode] {
    ...
    for num in 0..<count {
        ...
        let episode = Episode(id: id, name: name, season: season, episodeNumber: num + 1, thubnailImageURLString: thumbnail, description: desc, length: randLength, videoURL: exampleVideoURL)
